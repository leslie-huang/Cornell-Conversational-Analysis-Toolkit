...
class PromptTypes(Transformer):
    ...
    def __init__(
        ...
        ref_transform_field=None,
        prompt_filter=lambda utt, aux: True,
        ref_filter=lambda utt, aux: True,
        prompt_transform_filter=None,
        ref_transform_filter=None,
        ...
        self.prompt_transform_field = (
            prompt_transform_field
            if prompt_transform_field is not None
            else self.prompt_field
        )
        self.prompt_transform_filter = (
            prompt_transform_filter
            if prompt_transform_filter is not None
            else self.prompt_filter
        )
        self.ref_transform_field = (
            ref_transform_field if ref_transform_field is not None else self.ref_field
        )
        self.ref_transform_filter = (
            ref_transform_filter
            if ref_transform_filter is not None
            else self.ref_filter
        )
    ...
    def fit(self, corpus, y=None):
        """
			...
			:return: None
		"""
        _, prompt_input, _, ref_input = self._get_pair_input(
            corpus,
            self.prompt_field,
            self.ref_field,
            self.prompt_filter,
        ...
        self.prompt_embedding_model = fit_prompt_embedding_model(
            ...
            self.ref__tfidf_min_df,
            self.ref__tfidf_max_df,
            self.svd__n_components,
            self.random_state,
            self.verbosity,
        )
        self.train_results["prompt_ids"], self.train_results[
            "prompt_vects"
        ], self.train_results["ref_ids"], self.train_results[
            "ref_vects"
    ...
    def transform(self, corpus):
        ...
        corpus.set_vect_reprs(
            self.output_field + "__prompt_repr", prompt_ids, prompt_vects
        )
        corpus.set_vect_reprs(self.output_field + "__ref_repr", ref_ids, ref_vects)

        prompt_df, ref_df = self._get_type_assignments(
        ...
        ref_dists, ref_assigns = (
            ref_df[ref_df.columns[:-1]].values,
            ref_df["type_id"].values,
        )
        ...
        corpus.set_vect_reprs(
            self.output_field + "__ref_dists.%s" % self.default_n_types,
            ref_df.index,
            ref_dists,
        )
        for id, assign, dist in zip(prompt_df.index, prompt_assigns, prompt_min_dists):
            corpus.get_utterance(id).set_info(
        ...
        for id, assign, dist in zip(ref_df.index, ref_assigns, ref_min_dists):
            corpus.get_utterance(id).set_info(
                self.output_field + "__ref_type.%s" % self.default_n_types, assign
            )
            corpus.get_utterance(id).set_info(
                self.output_field + "__ref_type_dist.%s" % self.default_n_types,
                float(dist),
            )
        return corpus
    ...
    def _transform_utterance_side(self, utterance, side):
        ...
        utterance.set_info(
            self.output_field + "__%s_repr" % side, [float(x) for x in utt_vects[0]]
        )
        return utterance
    ...
    def _get_embeddings(self, corpus):
        prompt_ids, prompt_inputs = self._get_input(
            corpus, self.prompt_transform_field, self.prompt_transform_filter
        )
        ref_ids, ref_inputs = self._get_input(
            corpus, self.ref_transform_field, self.ref_transform_filter
        )
        prompt_ids, prompt_vects = transform_embeddings(
            self.prompt_embedding_model, prompt_ids, prompt_inputs, side="prompt"
        )
        ref_ids, ref_vects = transform_embeddings(
            self.prompt_embedding_model, ref_ids, ref_inputs, side="ref"
        )
    ...
    def _get_type_assignments(
        ...
        ref_df = assign_prompt_types(
            self.type_models[type_key], ref_ids, ref_vects, self.max_dist
        )
    ...
    def display_type(self, type_id, corpus=None, type_key=None, k=10):
        ...
        top_prompt = (
            prompt_df[prompt_df.type_id == type_id].sort_values(type_id).head(k)
        )
        ...
        if corpus is not None:
            ...
            top_prompt = (
                prompt_df[prompt_df.type_id == type_id]
                .sort_values(type_id)
                .head(k)
                .index
            )
            top_ref = (
                ref_df[ref_df.type_id == type_id].sort_values(type_id).head(k).index
            )
            print("top prompts:")
    ...
    def dump_model(self, model_dir, type_keys="default", dump_train_corpus=True):
        ...
        for k in ["prompt_tfidf_model", "ref_tfidf_model", "svd_model"]:
            joblib.dump(
                self.prompt_embedding_model[k], os.path.join(model_dir, k + ".joblib")
            )
        ...
        for key in to_dump:
            ...
            for k in ["prompt_df", "ref_df"]:
                type_model[k].to_csv(
                    os.path.join(model_dir, "%s.%s.tsv" % (k, key)), sep="\t"
                )
            if dump_train_corpus:
    ...
    def load_model(self, model_dir, type_keys="default", load_train_corpus=True):
        ...
        for k in ["prompt_tfidf_model", "ref_tfidf_model", "svd_model"]:
            self.prompt_embedding_model[k] = joblib.load(
                os.path.join(model_dir, k + ".joblib")
            )
        for k in ["U_prompt", "U_ref"]:
            self.prompt_embedding_model[k] = np.load(
                os.path.join(model_dir, k + ".npy")
            )
        ...
        if load_train_corpus:
            ...
            for k in ["prompt_ids", "prompt_vects", "ref_ids", "ref_vects"]:
                self.train_results[k] = np.load(
                    os.path.join(model_dir, "train_" + k + ".npy")
                )
    ...
    def _get_input(self, corpus, field, filter_fn, check_nonempty=True, aux_input={}):
        ...
        for utterance in corpus.iter_utterances():
            ...
            if isinstance(input, list):
                input = "\n".join(input)
            if filter_fn(utterance, aux_input) and (
    ...
    def _get_pair_input(
        ...
        prompt_field,
        ref_field,
        prompt_filter=lambda x, y, z: True,
        ref_filter=lambda x, y, z: True,
        check_nonempty=True,
        aux_input={},
    ):
        prompt_ids = []
        ...
        for ref_utt in corpus.iter_utterances():
            ...
            try:
                ...
                continue
            if prompt_filter(prompt_utt, aux_input) and ref_filter(ref_utt, aux_input):
                ...
                if (not check_nonempty) or (
                    (len(prompt_input) > 0) and (len(ref_input) > 0)
                ):
...
def fit_prompt_embedding_model(
    ...
    return {
        ...
        "ref_tfidf_model": ref_tfidf_model,
        "svd_model": svd_model,
...
def fit_prompt_type_model(
    ...
    ref_df = pd.DataFrame(
        index=model["ref_tfidf_model"].get_feature_names(),
        data=np.hstack([ref_dists, ref_clusters[:, np.newaxis]]),
        columns=list(range(n_types)) + ["type_id"],
    )
    return {"km_model": km, "prompt_df": prompt_df, "ref_df": ref_df}
...
def assign_prompt_types(model, ids, vects, max_dist=0.9):
    ...
    # this function is called for prompts and responses in same model, so use ids[1] to differentiate
