...
class PromptTypes(Transformer):
    ...
    def fit(
        ...
        self.train_results["prompt_ids"], self.train_results[
            "prompt_vects"
        ], self.train_results["reference_ids"], self.train_results[
            "reference_vects"
    ...
    def transform(
        ...
        corpus.set_vector_matrix(
            self.output_field + "__prompt_repr",
            matrix=prompt_vects,
            ids=prompt_ids,
        )
        corpus.set_vector_matrix(
        ...
        for id, assign, dist in zip(
            prompt_df.index, prompt_assigns, prompt_min_dists
        ):
            corpus.get_utterance(id).add_meta(
                self.output_field + "__prompt_type.%s" % self.default_n_types,
                assign,
            )
            corpus.get_utterance(id).add_meta(
                self.output_field
                + "__prompt_type_dist.%s" % self.default_n_types,
                float(dist),
            )
        for id, assign, dist in zip(
            reference_df.index, reference_assigns, reference_min_dists
        ):
            corpus.get_utterance(id).add_meta(
                self.output_field
                + "__reference_type.%s" % self.default_n_types,
                assign,
            )
            corpus.get_utterance(id).add_meta(
                self.output_field
                + "__reference_type_dist.%s" % self.default_n_types,
                float(dist),
            )
        return corpus
    ...
    def _transform_utterance_side(self, utterance, side):
        ...
        assign_df = assign_prompt_types(
            self.type_models[self.default_n_types],
            utt_ids,
            utt_vects,
            self.max_dist,
        )
        ...
        assign = vals[-1]
        utterance.add_meta(
            self.output_field + "__%s_type.%s" % (side, self.default_n_types),
            assign,
        )
        utterance.add_meta(
            self.output_field
            + "__%s_type_dist.%s" % (side, self.default_n_types),
            float(min_dist),
        )
        utterance.add_meta(
        ...
        utterance.add_meta(
            self.output_field + "__%s_repr" % side,
            [float(x) for x in utt_vects[0]],
        )
        return utterance
    ...
    def display_type(self, type_id, corpus=None, type_key=None, k=10):
        ...
        top_ref = (
            reference_df[reference_df.type_id == type_id]
            .sort_values(type_id)
            .head(k)
        )
        print("top prompt:")
        ...
        if corpus is not None:
            ...
            for utt in top_prompt:
                ...
                print(
                    corpus.get_utterance(utt).retrieve_meta(
                        self.prompt_transform_field
                    )
                )
                print()
    ...
    def dump_model(
        self, model_dir, type_keys="default", dump_train_corpus=True
    ):
        """
        ...
        if dump_train_corpus:
            ...
            for k in [
                ...
                np.save(
                    os.path.join(model_dir, "train_" + k),
                    self.train_results[k],
                )
    ...
    def load_model(
        self, model_dir, type_keys="default", load_train_corpus=True
    ):
        """
    ...
    def _get_pair_input(
        ...
        for reference_utt in corpus.iter_utterances():
            ...
            try:
                ...
                continue
            if prompt_selector(prompt_utt) and reference_selector(
                reference_utt
            ):
                ...
                if (not check_nonempty) or (
                    (len(prompt_input) > 0) and (len(reference_input) > 0)
                ):
...
def fit_prompt_embedding_model(
    ...
    svd_model = TruncatedSVD(
        n_components=svd__n_components,
...
def transform_embeddings(model, ids, input, side="prompt", filter_empty=True):
    ...
    tfidf_vects = normalize(
        model["%s_tfidf_model" % side].transform(input), norm="l1"
    )
...
def fit_prompt_type_model(
    model, n_types, random_state=None, max_dist=0.9, verbosity=0
):
    """
